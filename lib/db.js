...
var safe_db_commit = function (db) {//{{{
    "use strict";
    if (db.tx_status === 3) {
        console.log('[db]commit ' + db.db_name);
        db.tx_status = 4;
        db.exec('COMMIT');
        db.tx_status = undefined;
    } else {
        console.log('[db]commit conflict for ' + db.db_name + ' status is ' + db.tx_status);
    }
};//}}}
...
var safe_db_begin = function (db) {//{{{
    ...
    if (db.tx_status === undefined) {
        console.log('[db]begin transaction ' + db.db_name);
        db.tx_status = 1;
...
var init_writer = function (callback) {//{{{
    ...
    this.meta.get(sql.SELECT_META_SQL, [0], function (error, row) {
        ...
        self.volume.db_name = self.volume_file;
        self.volume.event = new Emitter();
        self.volume.event.setMaxListeners(constants.settings.MAX_EVENTS);
    ...
    self.volume.serialize(function () {
        ...
        self.volume.get(sql.GET_VOLUME_META, function (err, row) {
            ...
            after_volume_close = function () {//{{{
                ...
                origin_event = self.volume.event;
                self.volume = new sqlite3.Database(new_name);
                self.volume.db_name = self.volume_file;
                self.volume.event = origin_event;
                if (self.flush_writer_id !== undefined) {
                    safe_db_begin(self.volume);
                }
                self.volume.serialize(function () {
...
var DB = function (config, callback) {//{{{
    ...
    this.meta.db_name = 'meta';
    this.meta.event = new Emitter();
...
var insert = function (req_id, cmd, body, callback) {//{{{
    ...
    insert_volume = function () {//{{{
        //
        // if volume under begin transaction, 
        // then wait util begin finish.
        //
        if (self.volume.tx_status === 3) {
            console.log('[db] insert volume directly');
            self.volume.run(sql.INSERT_VOLUME_SQL,
                [req_id, cmd, body, new Date().getTime() / 1000],
                insert_callback);
        } else {
            console.log('[db] insert volume but wait begin event');
            self.volume.event.once('begin', function () {
                self.volume.run(sql.INSERT_VOLUME_SQL,
                    [req_id, cmd, body, new Date().getTime() / 1000],
                    insert_callback);
            });
...
var kill = function () {//{{{
    ...
    if (this.meta.flush_reader_id !== undefined) {
        clearInterval(this.meta.flush_reader_id);
        this.meta.flush_reader_id = undefined;
...
var flush_per_second = function () {//{{{
    ...
    this.flush_writer_id = setInterval(function () {
        ...
        if (self.cnt > 0) {
            console.log('[db]cnt is ' + self.cnt);
